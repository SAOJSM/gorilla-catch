...
function startDownload(tabId) {
    ...
    let blobUrl = "";   // 儲存blobURL
    let _blob = null;   // 儲存blob
    let downId = 0; // 下載的檔案ID
    ...
    if (Object.keys(_data).length > 0 && G.TitleName) {
        _data.title = stringModify(_data.title);    // 防止標題中有路徑分隔符 導致下載新建資料夾
