...
const _fileName = params.get("filename");   // 自定義檔案名
let tsAddArg = params.get("tsAddArg");  // 自定義切片參數
let autoReferer = params.get("autoReferer");    // 是否已經自動調整 referer
const tabId = parseInt(params.get("tabid"));    // 資源所在的標籤頁ID 用來獲取密鑰
const key = params.get("key");  // 自定義密鑰
...
/*
*   popup 合併多個m3u8 需要提交以下參數 ffmpeg才能判斷檔案是否添加完畢
...
// 修改當前標籤下的所有xhr的Referer 修改完成 運行init函數
...
let possibleKeys = new Set();   // 儲存疑似密鑰
let downId = 0; // chrome下載api 回調id

/* 以下參數 新下載器已棄用 */
let stopDownload = false; // 停止下載flag
let downDuration = 0; // 下載媒體的時長
let downSize = 0;
let downCurrentTs = 0;    // 當前進度
let downTotalTs = 0;  // 需要下載的檔案數量
let tsBuffer = []; // ts內容緩存
const errorTsList = []; // 下載錯誤ts序號列表

let fileStream = undefined; // 流式下載檔案輸出流
const downSet = {};   // 下載時儲存設定
...
/* DOM */
const $fileSize = $("#fileSize");   // 下載檔案大小進度
...
/**
 * 初始化函數，介面預設配置 loadSource載入 m3u8 url
 */
function init() {
    // 獲取頁面DOM
    if (tabId && tabId != -1) {
...
/* 來自 監聽 LEVEL_LOADED 提取所有ts連結 進一步處理 */
function parseTs(data) {
    // console.log(data);
    let isEncrypted = false;
    _fragments.splice(0);   // 清空 防止直播HLS無限添加
    /* 獲取 m3u8文件原始內容 MANIFEST_PARSED也能獲取但偶爾會為空(BUG?) 放在LEVEL_LOADED獲取更安全*/
    ...
    for (let i in data.fragments) {
        ...
        */
        ...
        /* 
        * 查看是否加密 下載key
        * firefox CSP政策不允許在script-src 使用blob 不能直接調用hls.js下載好的密鑰
        */
                ...
                fetch(data.fragments[i].decryptdata.uri)
                    ...
                    .then(function (buffer) {
                        if (buffer.byteLength == 16) {
                            keyContent.set(data.fragments[i].decryptdata.uri, buffer); // 儲存密鑰
    ...
    // 錄製直播
    /* 直播是持續更新的m3u8 recorderLast保存下載的最後一個url 以便下次更新時判斷從哪個切片開始繼續下載 */
    if (recorder) {
...
function downloadTs(start = 0, end = _fragments.length - 1, errorObj = undefined) {
    ...
    const _tsThread = parseInt($("#thread").val());  // 原始線程數量
    let tsThread = _tsThread;  // 線程數量
    let index = start - 1; // 當前下載的索引
    ...
    const tsInterval = setInterval(function () {
        ...
        if (index == end && tsThread == _tsThread) {
            ...
            $progress.html(`數據不完整... 剩余未下載: ${errorTsList.length}`);
            return;
        }
        // 下載
        if (tsThread > 0 && index < end) {
            tsThread--;
            let currentIndex = ++index;   // 當前下載的索引
            ...
            $.ajax({
                ...
                if (recorder) {
                    $fileDuration.html("錄制時長:" + secToTime(downDuration));
                    return;
                }
                downCurrentTs++;
...
function downloadNew(start = 0, end = _fragments.length) {
    ...
    // 轉碼函數 如果存在down.mapTag 跳過轉碼
    if (downSet.mp4 && !down.mapTag) {
        let tempBuffer = null;
        let head = true;
        transmuxer = new muxjs.mp4.Transmuxer({ remux: !downSet.onlyAudio });    // mux.js 物件
    ...
    down.fragments.forEach((fragment) => {
        ...
        $button.click(function () {
            ...
            if ($(this).data("action") == "stop") {
                ...
                down.downloader();  // 停止當前下載器 重新開一個下載器保持線程數量
...
function mergeTsNew(down) {
    ...
    if ($("#ffmpeg").prop("checked") || _ffmpeg || isSendFfmpeg) {
         ...
         * chrome每個進程限制2G內存 處理2G視頻可能導致超過限制。1.8G是安全值。
         * firefox 不受影響
         */
...
function streamDownload(start = 0, end = _fragments.length - 1) {
    ...
    const tsInterval = setInterval(function () {
        ...
        // 下載指針超過推流指針太多(超過線程數) 暫停下載
        if (index - downP > _tsThread) { return; }
        // 還有線程數 並且下載指針小於總下載量 開啟下載
        if (tsThread > 0 && index < downTotalTs) {
            tsThread--;
            const currentIndex = index++; // 記錄當前下載指針
            $.ajax({
                ...
                if (stopDownload) { return; }
                // 解密需要當前資源的總索引 downList[currentIndex].index
...
function initDownload() {
    ...
    $fileDuration.html("");
    tsBuffer.splice(0); // 初始化一下載ts緩存
    downCurrentTs = 0;  // 當前進度
    stopDownload = false; // 停止下載
    recorderIndex = 0;  // 錄製直播索引
    recorderLast = "";  // 錄製最後下載的url
    fileStream = undefined; // 流式下載 文件流
    // 轉碼工具初始化
    transmuxer = undefined;
    transmuxerStatus = false;
    transmuxerheadEncode = undefined;
    // 避免下載中途 更改設置 暫時儲存下載配置
...
function getM3u8DlREArg() {
    ...
    const addParam = $("#addParam").prop("checked");    // 是否添加參數
    const customFilename = $("#customFilename").val().trim();   // 自定義文件名
    ...
    // 線程處理
    const tsThread = $("#thread").val();  // 線程數量
    ...
    // 自定義密鑰
    let customKey = $("#customKey").val().trim();  // 自定義密鑰
    if (customKey) {
