// 低版本chrome manifest v3協議 會有 getMessage 函式不存在的bug
 ...
 * 例如 夸克瀏覽器
 * 使用傳統下載方式下載 但無法監聽 無法另存為 無法判斷下載是否失敗 唉~
 */
if (!chrome.downloads) {
    chrome.downloads = {
...
G.blackList = new Set();    // 正則遮蔽資源列表
G.blockUrlSet = new Set();    // 遮蔽網址列表
G.requestHeaders = new Map();   // 臨時儲存請求頭
// 初始化目前tabId
...
function InitOptions() {
    // 斷開重新連線后 立刻把local里MediaData數據交給cacheData
...
/**
 * 將使用者輸入的URL（可能包含萬用字元）轉換為正規表示式
 * @param {string} urlPattern - 使用者輸入的URL，可能包含萬用字元
 * @returns {RegExp} - 轉換后的正規表示式
 */
function wildcardToRegex(urlPattern) {
    ...
    const regexPattern = urlPattern
        .replace(/[.+^${}()|[\]\\]/g, '\\$&') // 轉義正規表示式特殊字元
        .replace(/\*/g, '.*') // 將 * 替換為 .*
        .replace(/\?/g, '.'); // 將 ? 替換為 .

    // 建立正規表示式，確保匹配整個URL
    return new RegExp(`^${regexPattern}$`, 'i'); // 忽略大小寫
}